...
function getPresentationVariantForLineItem(uIAnnotations, settings, logger) {
    ...
    if (annotationPath) {
        ...
        const accessor = isSPV || isPV ? annotationPath.replace(UIVOCABULARYDOT, '') : undefined;
        const uiAnnotation = accessor ? uIAnnotations[accessor] : undefined; // the ui annotation specified by the annotationPath if it exists
        if (uiAnnotation) {
            if (isPV) {
